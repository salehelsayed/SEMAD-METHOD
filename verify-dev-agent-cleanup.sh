...
echo "Verifying dev agent has no legacy memory references..."
echo "=================================================="

# Check for any remaining legacy memory references
echo -e "\n1. Checking for memory system references:"
grep -i -E "memory-manager|persist-memory|loadMemory|saveMemory|retrieveRelevant|memory-usage-logger|memory-cli|dev-save-memory" bmad-core/agents/dev.md | grep -v "# Removed:" | grep -v "data/content:" || echo "✅ No legacy memory references found"
...
grep -A 20 "dependencies:" bmad-core/agents/dev.md | grep -i "memory" | grep -v "# Removed:" || echo "✅ No legacy memory dependencies found"
